...
// Tickers sem preço de mercado (cash, equivalentes)
const NO_PRICE_TICKERS = new Set(["CASH", "BRL", "USD"]);

function calcNewMarketValue(
  ...
  // primeira inicialização com preço real: recalcula via quantidade
  if (qty !== null && qty > 0) {
    return qty * newPrice;
  }
  return oldMv ?? 0;
...
export async function refreshPositionPrices(): Promise<PriceUpdateResult> {
  ...
  // Mapa yahooTicker → posições; separa cash/equivalentes para FX-only
  const tickerToPositions = new Map<string, PosData[]>();
  ...
  const allTickers = [...tickerToPositions.keys()];
  const [quotes, fxRate] = await Promise.all([
    fetchYahooQuotes(allTickers).catch((e: Error) => {
      errors.push(`Yahoo Finance: ${e.message}`);
      return new Map<string, { ticker: string; price: number; changePercent: number | null; marketState: string }>();
    }),
  ...
  for (const [yahooTicker, posList] of tickerToPositions.entries()) {
    ...
    for (const pos of posList) {
      ...
      const newMvBrl = pos.currency === "USD" ? newMv * fxRate : newMv;
  ...
  // Posições USD sem ticker e CASH: atualiza só market_value_brl pelo câmbio
